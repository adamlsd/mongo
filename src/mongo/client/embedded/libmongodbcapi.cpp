...
namespace
{
    ...
    {
        void
        operator()( ::mongo::ServiceContext *const serviceContext ) const noexcept
        {
        ...
        if( this->logCallbackHandle )
        {
            ...
            this->logCallbackHandle.reset();
                ...
                [] ( ) noexcept {
                    ...
                    if( globalLogManager()->isDefaultConsoleAppenderAttached() ) globalLogManager()->detachDefaultConsoleAppender(); }( );
                throw;
    }

    libmongodbcapi_lib( libmongodbcapi_init_params *const params )= default;

        // XXX XXX XXX Ask Henrik about this logic here.  What doe we want the state to be when we start, what do we want it to be when we end?
                ...
                if( params )
                {
                    ...
                    {
                        if( !globalLogManager()->isDefaultConsoleAppenderAttached() ) globalLogManager()->
                ...
                lib->logCallbackHandle= logger::globalLogDomain()->attachAppender(
                    std::make_unique< embedded::EmbeddedLogAppender< logger::MessageEventEphemeral > >( logCallback, logUserData, std::make_unique< logger::MessageEventUnadornedEncoder >() ) );
                    }
                }
...
namespace mongo
{
    namespace
    {
        ...
        {
            public:
                explicit MobileException( const libmongodbcapi_error code, std::string m ) : _mesg( std::move( m ) ), _code( code ) {}

                libmongodbcapi_error mobileCode() const noexcept { return this->_code; }
        ...
        namespace capi
        {
            ...
            void
            registerLogCallback( libmongodbcapi_lib *const lib, const libmongodbcapi_log_callback logCallback, void *const logUserData )
            {
                lib->logCallbackHandle= logger::globalLogDomain()->attachAppender(
                    std::make_unique< embedded::EmbeddedLogAppender< logger::MessageEventEphemeral > >( logCallback, logUserData, std::make_unique< logger::MessageEventUnadornedEncoder >() ) );
            }
            ...
            libmongodbcapi_lib *
            lib_init( libmongodbcapi_init_params const *params )
            try
            {
                if( library )
                {
                ...
                auto lib= std::make_unique< libmongodbcapi_lib >( params );
                ...
                if( params )
                {
                    ...
                    if( params->log_flags & LIBMONGODB_CAPI_LOG_STDOUT )
                    {
                        if( !globalLogManager()->isDefaultConsoleAppenderAttached() ) globalLogManager()->reattachDefaultConsoleAppender();
                    }
                    else
                    {
                        if( globalLogManager()->isDefaultConsoleAppenderAttached() ) globalLogManager()->detachDefaultConsoleAppender();
            ...
            void
            capi_lib_fini( libmongodbcapi_lib *const lib )
            {
                if( !lib )
                {
                    throw MobileException{
            ...
            libmongodbcapi_instance *
            instance_create( libmongodbcapi_lib *const lib, const char *const yaml_config )
            {
                if( !library )
                {
            ...
            void
            instance_destroy( libmongodbcapi_instance *const db )
            {
                if( !library )
                {
                    throw MobileException{ LIBMONGODB_CAPI_ERROR_LIBRARY_NOT_INITIALIZED,
                                           "Cannot destroy a database handle when the MongoDB Embedded Library is not yet initialized." };
                }
            ...
            libmongodbcapi_client *
            client_create( libmongodbcapi_instance *const db )
            {
                if( !library )
                {
                    throw MobileException{ LIBMONGODB_CAPI_ERROR_LIBRARY_NOT_INITIALIZED,
                                           "Cannot create a new client handle when the MongoDB Embedded Library is not yet initialized." };
                }

                if( !db )
                {
                    throw MobileException{ LIBMONGODB_CAPI_ERROR_INVALID_DB_HANDLE,
                                           "Cannot use a `NULL` pointer referencing a MongoDB Embedded Database when creating a new client" };
                }

                if( db != library->onlyDB.get() )
                {
                    throw MobileException{ LIBMONGODB_CAPI_ERROR_INVALID_DB_HANDLE,
                                           "The specified MongoDB Embedded Database instance cannot be used to create a new client because it is invalid." };
                }
            ...
            void
            client_destroy( libmongodbcapi_client *const client )
            {
                if( !library )
                {
                    throw MobileException( LIBMONGODB_CAPI_ERROR_LIBRARY_NOT_INITIALIZED,
            ...
            void
            client_invoke( libmongodbcapi_client *const client, const void *input, const size_t input_size, void **const output, size_t *const output_size )
            {
                ...
                // The results of the computations used to fill out-parameters need to be captured and processed before setting the output parameters themselves, in order to maintain the strong-guarantee part of the contract of this function.
                auto outParams= std::make_tuple( client->response.response.size(), client->response.response.buf() );

                // We force the output parameters to be set in a `noexcept` enabled way.  If the operation itself is safely noexcept, we just run it, otherwise we force a `noexcept` over it to catch errors.
                ...
                {
                    // Assigning primitives in a tied tuple should be noexcept, so we force it to be so, for our purposes.  This facilitates a runtime check should something WEIRD happen.
                    [ output, output_size, &outParams ]() noexcept { std::tie( *output_size, *output )= std::move( outParams ); } ( );
                }
            }

            int
            status_get_error( const libmongodbcapi_status *const status ) noexcept
            {
                invariant( status );
                return status->error;
            }
            ...
            const char *
            status_get_what( const libmongodbcapi_status *const status ) noexcept
            {
                invariant( status );
            ...
            int
            status_get_code( const libmongodbcapi_status *const status ) noexcept
            {
                invariant( status );
        ...
        class ReentrancyGuard
        {
            public:
                ...
                ~ReentrancyGuard() { inLibrary= false; }
        ...
        std::nullptr_t
        handleException( libmongodbcapi_status &status ) noexcept
        {
            try
            {
        ...
        template< typename Function, typename ReturnType= decltype( std::declval< Function >( )( *std::declval< libmongodbcapi_status * >( ) ) ) >
        struct enterCXXImpl;
        ...
        struct enterCXXImpl< Function, void >
        {
            ...
            static int
            call( Callable &&function, libmongodbcapi_status &status ) noexcept
            {
                try
                {
...
namespace
{
    struct StatusGuard
    {
        ...
        public:
            explicit StatusGuard( libmongodbcapi_status *const statusPtr ) noexcept : status( statusPtr ? statusPtr : &fallback )
            {
                status->clean();
            }

            libmongodbcapi_status &get() noexcept { return *status; }

            const libmongodbcapi_status &get() const noexcept { return *status; }

            operator libmongodbcapi_status &() & noexcept { return this->get(); }
            operator libmongodbcapi_status &() && noexcept { return this->get(); }
    };

    template< typename Callable >
    auto
    enterCXX( libmongodbcapi_status *const statusPtr, Callable &&c ) noexcept
    {
...
{
    libmongodbcapi_lib *
    libmongodbcapi_lib_init( const libmongodbcapi_init_params *const params, libmongodbcapi_status *const statusPtr )
    {
        return enterCXX( statusPtr, [ & ] { return mongo::capi::lib_init( params ); } );
    }

    int
    libmongodbcapi_lib_fini( libmongodbcapi_lib *const lib, libmongodbcapi_status *const statusPtr )
    {
        return enterCXX( statusPtr, [ & ] { return mongo::capi::lib_fini( lib );
        } );
    }

    libmongodbcapi_instance *
    libmongodbcapi_instance_create( libmongodbcapi_lib *lib, const char *const yaml_config, libmongodbcapi_status *const statusPtr )
    {
        return enterCXX( statusPtr, [ & ] { return mongo::capi::instance_create( lib, yaml_config ); } );
    }

    int
    libmongodbcapi_instance_destroy( libmongodbcapi_instance *const db, libmongodbcapi_status *const statusPtr )
    {
        return enterCXX( statusPtr, [ & ] { return mongo::capi::instance_destroy( db ); } );
    }

    libmongodbcapi_client *
    libmongodbcapi_client_create( libmongodbcapi_instance *const db, libmongodbcapi_status *const statusPtr )
    {
        return enterCXX( statusPtr, [ & ] { return mongo::capi::client_create( db ); } );
    }

    int
    libmongodbcapi_client_destroy( libmongodbcapi_client *const client, libmongodbcapi_status *const statusPtr )
    {
        return enterCXX( statusPtr, [ & ] { return mongo::capi::client_destroy( client ); } );
    }

    int
    libmongodbcapi_client_invoke( libmongodbcapi_client *const client, const void *input, const size_t input_size, void **const output, size_t *const output_size, libmongodbcapi_status *const statusPtr )
    {
        return enterCXX( statusPtr, [ & ] { return mongo::capi::client_invoke( client, input, input_size, output, output_size ); } );
    }

    int
    libmongodbcapi_status_get_error( const libmongodbcapi_status *const status )
    {
        return enterCXX( nullptr, [&]{ return mongo::capi::status_get_error( status ); } );
    }

    const char *
    libmongodbcapi_status_get_explanation( const libmongodbcapi_status *const status )
    {
        return enterCXX( nullptr, [&]{ return  mongo::capi::status_get_what( status ); } );
    }

    int
    libmongodbcapi_status_get_code( const libmongodbcapi_status *const status )
    {
        return enterCXX( nullptr, [&]{ return mongo::capi::status_get_code( status ); } );
    }

    libmongodbcapi_status *
    libmongodbcapi_status_create()
    {
        return enterCXX( nullptr, [ & ] { return mongo::capi::status_create(); } );
    }

    void
    libmongodbcapi_status_destroy( libmongodbcapi_status *const status )
    {
        return enterCXX( nullptr, [ & ] { return mongo::capi::status_destroy( status ); } );
    }
}  // extern "C"
