...
namespace mongo
{
    ...
    class CollectionInfoCache
    {
		...
		private:
			...
			// This structure exists to give us a customization point to decide how to force users of this class to depend upon the corresponding `collection_info_cache.cpp` Translation Unit (TU).  All public forwarding functions call `_impl(), and `_impl` creates an instance of this structure.
			struct TUHook
			{
		...
		public:
            ...
            explicit inline CollectionInfoCache( Collection *const collection, const NamespaceString &ns ) : _pimpl( makeImpl( collection, ns ) ) {}
             ...
             */
            inline void init( OperationContext *const opCtx ) { return this->_impl().init( opCtx ); }

            /**
             * Get the PlanCache for this collection.
             */
            inline PlanCache *getPlanCache() const { return this->_impl().getPlanCache(); }

            /**
             * Get the QuerySettings for this collection.
             */
            inline QuerySettings *getQuerySettings() const { return this->_impl().getQuerySettings(); }

            /* get set of index keys for this namespace.  handy to quickly check if a given
               field is indexed (Note it might be a secondary component of a compound index.)
            */
            inline const UpdateIndexData &getIndexKeys( OperationContext *const opCtx ) const { return this->_impl().getIndexKeys( opCtx ); }
             ...
             */
            inline CollectionIndexUsageMap getIndexUsageStats() const { return this->_impl().getIndexUsageStats(); }
             ...
             */
            inline void addedIndex( OperationContext *const opCtx, const IndexDescriptor *const desc ) { return this->_impl().addedIndex( opCtx, desc ); }
             ...
             * Must be called under exclusive collection lock.
             */
            inline void droppedIndex( OperationContext *const opCtx, const StringData indexName ) { return this->_impl().droppedIndex( opCtx, indexName ); }

            /**
             * Removes all cached query plans.
             */
            inline void clearQueryCache() { return this->_impl().clearQueryCache(); }
             ...
             */
            inline void notifyOfQuery( OperationContext *const opCtx, const std::set< std::string > &indexesUsed ) { return this->_impl().notifyOfQuery( opCtx, indexesUsed ); }
    };
}  // namespace mongo
