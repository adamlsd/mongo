...
MONGO_INITIALIZER(InitializeCollectionInfoCacheFactory)(InitializerContext* const) {
    CollectionInfoCache::registerImpl([](Collection* const collection) {
        return stdx::make_unique<CollectionInfoCacheImpl>(collection);
    });
    return Status::OK();
}
}  // namespace

CollectionInfoCacheImpl::CollectionInfoCacheImpl(Collection* collection)
...
const UpdateIndexData& CollectionInfoCacheImpl::getIndexKeys(OperationContext* txn) const {
...
void CollectionInfoCacheImpl::computeIndexKeys(OperationContext* txn) {
    _indexedPaths.clear();
...
void CollectionInfoCacheImpl::notifyOfQuery(OperationContext* txn,
...
void CollectionInfoCacheImpl::updatePlanCacheIndexEntries(OperationContext* txn) {
...
void CollectionInfoCacheImpl::init(OperationContext* txn) {
...
void CollectionInfoCacheImpl::addedIndex(OperationContext* txn, const IndexDescriptor* desc) {
    // Requires exclusive collection lock.
    invariant(txn->lockState()->isCollectionLockedForMode(_collection->ns().ns(), MODE_X));
    invariant(desc);
...
void CollectionInfoCacheImpl::droppedIndex(OperationContext* txn, StringData indexName) {
    // Requires exclusive collection lock.
    invariant(txn->lockState()->isCollectionLockedForMode(_collection->ns().ns(), MODE_X));
...
void CollectionInfoCacheImpl::rebuildIndexData(OperationContext* txn) {
    clearQueryCache();
