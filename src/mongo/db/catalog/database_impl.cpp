...
namespace mongo {
namespace {
MONGO_INITIALIZER(InitializeDatabaseImplFactory)(InitializerContext* const) {
    Database::registerImpl([](Database* const this_,
                              OperationContext* const txn,
                              const StringData name,
                              DatabaseCatalogEntry* const dbEntry) {
        return stdx::make_unique<DatabaseImpl>(this_, txn, name, dbEntry);
    });
    return Status::OK();
}
}  // namespace
...
public:
    AddCollectionChange(OperationContext* txn, DatabaseImpl* db, StringData ns)
        : _txn(txn), _db(db), _ns(ns.toString()) {}

    virtual void commit() {
        CollectionMap::const_iterator it = _db->collections().find(_ns);
...
void DatabaseImpl::close(Database* this_, OperationContext* txn) {
    ...
    repl::oplogCheckCloseDatabase(txn, this_);
...
Collection* DatabaseImpl::_getOrCreateCollectionInstance(OperationContext* txn, StringData fullns) {
...
void DatabaseImpl::clearTmpCollections(OperationContext* txn) {
...
Status DatabaseImpl::setProfilingLevel(Database* this_, OperationContext* txn, int newLevel) {
    ...
    Status status = createProfileCollection(txn, this_);
    if (!status.isOK()) {
        return status;
    }
...
Status DatabaseImpl::dropView(OperationContext* txn, StringData fullns) {
    Status status = _views.dropView(txn, NamespaceString(fullns));
    Top::get(txn->getClient()->getServiceContext()).collectionDropped(fullns);
    return status;
}

Status DatabaseImpl::dropCollection(OperationContext* txn, StringData fullns) {
...
Status DatabaseImpl::dropCollectionEvenIfSystem(OperationContext* txn,
...
void DatabaseImpl::_clearCollectionCache(OperationContext* txn,
                                         StringData fullns,
...
Status DatabaseImpl::renameCollection(OperationContext* txn,
                                      StringData fromNS,
                                      StringData toNS,
                                      bool stayTemp) {
...
Collection* DatabaseImpl::getOrCreateCollection(OperationContext* txn, StringData ns) {
...
Status DatabaseImpl::createView(OperationContext* txn,
                                StringData ns,
                                const CollectionOptions& options) {
    invariant(txn->lockState()->isDbLockedForMode(name(), MODE_X));
...
Collection* DatabaseImpl::createCollection(OperationContext* txn,
                                           StringData ns,
                                           const CollectionOptions& options,
                                           bool createIdIndex,
