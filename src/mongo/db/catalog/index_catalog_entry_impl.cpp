...
MONGO_INITIALIZER(InitializeIndexCatalogEntryFactory)(InitializerContext* const) {
    IndexCatalogEntry::registerFactory([](OperationContext* const txn,
                                          const StringData ns,
                                          CollectionCatalogEntry* const collection,
                                          IndexDescriptor* const descriptor,
                                          CollectionInfoCache* const infoCache) {
        return stdx::make_unique<IndexCatalogEntryImpl>(txn, ns, collection, descriptor, infoCache);
    });
    return Status::OK();
}
}  // namespace
...
public:
    HeadManagerImpl(IndexCatalogEntryImpl* ice) : _catalogEntry(ice) {}
...
const RecordId& IndexCatalogEntryImpl::head(OperationContext* txn) const {
    DEV invariant(_head == _catalogHead(txn));
    return _head;
}

bool IndexCatalogEntryImpl::isReady(OperationContext* txn) const {
...
MultikeyPaths IndexCatalogEntryImpl::getMultikeyPaths(OperationContext* txn) const {
    stdx::lock_guard<stdx::mutex> lk(_indexMultikeyPathsMutex);
...
void IndexCatalogEntryImpl::setHead(OperationContext* txn, RecordId newHead) {
...
void IndexCatalogEntryImpl::setMultikey(OperationContext* txn, const MultikeyPaths& multikeyPaths) {
...
bool IndexCatalogEntryImpl::_catalogIsReady(OperationContext* txn) const {
    return _collection->isIndexReady(txn, _descriptor->indexName());
}

RecordId IndexCatalogEntryImpl::_catalogHead(OperationContext* txn) const {
    return _collection->getIndexHead(txn, _descriptor->indexName());
}

bool IndexCatalogEntryImpl::_catalogIsMultikey(OperationContext* txn,
...
const IndexCatalogEntryImpl* IndexCatalogEntryContainer::find(
    const IndexDescriptor* desc) const {
    if (desc->_cachedEntry)
        return desc->_cachedEntry;

    for (const_iterator i = begin(); i != end(); ++i) {
        const IndexCatalogEntryImpl* e = *i;
...
IndexCatalogEntryImpl* IndexCatalogEntryContainer::find(const IndexDescriptor* desc) {
    if (desc->_cachedEntry)
        return desc->_cachedEntry;

    for (iterator i = begin(); i != end(); ++i) {
        IndexCatalogEntryImpl* e = *i;
        if (e->descriptor() == desc)
            return e;
    }
    return NULL;
...
IndexCatalogEntryImpl* IndexCatalogEntryContainer::find(const string& name) {
    for (iterator i = begin(); i != end(); ++i) {
        IndexCatalogEntryImpl* e = *i;
...
IndexCatalogEntryImpl* IndexCatalogEntryContainer::release(const IndexDescriptor* desc) {
    for (std::vector<IndexCatalogEntryImpl*>::iterator i = _entries.mutableVector().begin();
