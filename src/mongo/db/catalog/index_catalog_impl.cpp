...
Status IndexCatalogImpl::init(OperationContext* txn) {
...
IndexCatalogEntry* IndexCatalogImpl::_setupInMemoryStructures(OperationContext* txn,
                                                              IndexDescriptor* descriptor,
                                                              bool initFromDisk) {
...
void IndexCatalogImpl::_checkMagic() const {
    ...
    log() << "IndexCatalogImpl::_magic wrong, is : " << _magic;
    fassertFailed(17198);
}
...
Status IndexCatalogImpl::checkUnfinished() const {
    ...
    return Status(ErrorCodes::InternalError,
                  str::stream() << "IndexCatalogImpl has left over indexes that must be cleared"
                                << " ns: "
                                << _collection->ns().ns());
}

bool IndexCatalogImpl::_shouldOverridePlugin(OperationContext* txn,
...
string IndexCatalogImpl::_getAccessMethodName(OperationContext* txn,
                                              const BSONObj& keyPattern) const {
...
Status IndexCatalogImpl::_upgradeDatabaseMinorVersionIfNeeded(OperationContext* txn,
...
StatusWith<BSONObj> IndexCatalogImpl::prepareSpecForCreate(OperationContext* txn,
...
StatusWith<BSONObj> IndexCatalogImpl::createIndexOnEmptyCollection(OperationContext* txn,
                                                                   BSONObj spec) {
...
IndexCatalogImpl::IndexBuildBlock::IndexBuildBlock(OperationContext* txn,
                                                   Collection* collection,
                                                   const BSONObj& spec)
    : _collection(collection),
      _catalog(collection->getIndexCatalog()),
      _ns(_catalog->collection()->ns().ns()),
      _spec(spec.getOwned()),
      _entry(NULL),
      _txn(txn) {
...
Status IndexCatalogImpl::_isSpecOk(OperationContext* txn, const BSONObj& spec) const {
...
Status IndexCatalogImpl::_doesSpecConflictWithExisting(OperationContext* txn,
...
Status IndexCatalogImpl::dropAllIndexes(OperationContext* txn, bool includingIdIndex) {
    invariant(txn->lockState()->isCollectionLockedForMode(_collection->ns().toString(), MODE_X));
...
Status IndexCatalogImpl::dropIndex(OperationContext* txn, IndexDescriptor* desc) {
    invariant(txn->lockState()->isCollectionLockedForMode(_collection->ns().toString(), MODE_X));
    IndexCatalogEntry* entry = _entries.find(desc);
...
Status IndexCatalogImpl::_dropIndex(OperationContext* txn, IndexCatalogEntry* entry) {
    /**
     * IndexState in order
     *  <db>.system.indexes
     *    NamespaceDetails
    ...
    // ----- SANITY CHECKS -------------
    if (!entry)
        return Status(ErrorCodes::BadValue, "IndexCatalogImpl::_dropIndex passed NULL");
...
void IndexCatalogImpl::_deleteIndexFromDisk(OperationContext* txn,
...
vector<BSONObj> IndexCatalogImpl::getAndClearUnfinishedIndexes(OperationContext* txn) {
...
bool IndexCatalogImpl::isMultikey(OperationContext* txn, const IndexDescriptor* idx) {
    IndexCatalogEntry* entry = _entries.find(idx);
    invariant(entry);
    return entry->isMultikey();
}

MultikeyPaths IndexCatalogImpl::getMultikeyPaths(OperationContext* txn,
...
int IndexCatalogImpl::numIndexesTotal(OperationContext* txn) const {
...
IndexDescriptor* IndexCatalogImpl::IndexIterator::next() {
    if (!more())
        return NULL;
    _prev = _next;
    _advance();
...
IndexAccessMethod* IndexCatalogImpl::IndexIterator::accessMethod(const IndexDescriptor* desc) {
    invariant(desc == _prev->descriptor());
    return _prev->accessMethod();
}

IndexCatalogEntry* IndexCatalogImpl::IndexIterator::catalogEntry(const IndexDescriptor* desc) {
    invariant(desc == _prev->descriptor());
    return _prev;
}
...
void IndexCatalogImpl::findIndexesByKeyPattern(OperationContext* txn,
                                               const BSONObj& key,
                                               bool includeUnfinishedIndexes,
...
IndexDescriptor* IndexCatalogImpl::findShardKeyPrefixedIndex(OperationContext* txn,
...
void IndexCatalogImpl::findIndexByType(OperationContext* txn,
                                       const string& type,
...
const IndexDescriptor* IndexCatalogImpl::refreshEntry(OperationContext* txn,
    ...
    // Notify other users of the IndexCatalogImpl that we're about to invalidate 'oldDesc'.
...
Status IndexCatalogImpl::_indexFilteredRecords(OperationContext* txn,
...
Status IndexCatalogImpl::_indexRecords(OperationContext* txn,
                                       IndexCatalogEntry* index,
                                       const std::vector<BsonRecord>& bsonRecords,
                                       int64_t* keysInsertedOut) {
...
Status IndexCatalogImpl::_unindexRecord(OperationContext* txn,
                                        IndexCatalogEntry* index,
                                        const BSONObj& obj,
                                        const RecordId& loc,
                                        bool logIfError,
...
Status IndexCatalogImpl::indexRecords(OperationContext* txn,
                                      const std::vector<BsonRecord>& bsonRecords,
                                      int64_t* keysInsertedOut) {
    if (keysInsertedOut) {
        *keysInsertedOut = 0;
...
void IndexCatalogImpl::unindexRecord(OperationContext* txn,
                                     const BSONObj& obj,
                                     const RecordId& loc,
                                     bool noWarn,
                                     int64_t* keysDeletedOut) {
...
void IndexCatalogImpl::prepareInsertDeleteOptions(OperationContext* txn,
                                                  const IndexDescriptor* desc,
                                                  InsertDeleteOptions* options) {
    auto replCoord = repl::ReplicationCoordinator::get(txn);
...
StatusWith<BSONObj> IndexCatalogImpl::_fixIndexSpec(OperationContext* txn,
                                                    Collection* collection,
                                                    const BSONObj& spec) {
    ...
    // We've already verified in IndexCatalogImpl::_isSpecOk() that the index version is present and
