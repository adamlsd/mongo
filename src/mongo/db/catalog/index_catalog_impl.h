...
public:
    IndexCatalogImpl(Collection* collection);
    ...
    int numIndexesTotal(OperationContext* txn) const override;
    int numIndexesReady(OperationContext* txn) const override;
    ...
    bool haveIdIndex(OperationContext* txn) const override;
    ...
    IndexDescriptor* findIdIndex(OperationContext* txn) const override;
     ...
     */
    StatusWith<BSONObj> createIndexOnEmptyCollection(OperationContext* txn, BSONObj spec) override;

    StatusWith<BSONObj> prepareSpecForCreate(OperationContext* txn,
                                             const BSONObj& original) const override;

    Status dropAllIndexes(OperationContext* txn, bool includingIdIndex) override;

    Status dropIndex(OperationContext* txn, IndexDescriptor* desc) override;

    /**
     * will drop all incompleted indexes and return specs
     * after this, the indexes can be rebuilt
     */
    std::vector<BSONObj> getAndClearUnfinishedIndexes(OperationContext* txn) override;
     ...
     */
    bool isMultikey(OperationContext* txn, const IndexDescriptor* idx) override;
     ...
     */
    MultikeyPaths getMultikeyPaths(OperationContext* txn, const IndexDescriptor* idx) override;
    ...
    public:
        ...
        IndexCatalogEntry* getEntry() override {
            return _entry;
        }
    ...
    std::string getAccessMethodName(OperationContext* txn, const BSONObj& keyPattern) override {
    ...
    inline Status upgradeDatabaseMinorVersionIfNeeded(OperationContext* const txn,
        ...
        return _upgradeDatabaseMinorVersionIfNeeded(txn, newPluginName);
    }
