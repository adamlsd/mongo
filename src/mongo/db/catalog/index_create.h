...
namespace mongo
{
    ...
    class MultiIndexBlock
    {
		...
		private:
			...
			// This structure exists to give us a customization point to decide how to force users of this class to depend upon the corresponding `index_create.cpp` Translation Unit (TU).  All public forwarding functions call `_impl(), and `_impl` creates an instance of this structure.
			struct TUHook
			{
		...
		public:
             ...
             */
            inline explicit MultiIndexBlock( OperationContext *const opCtx, Collection *const collection ) : _pimpl( makeImpl( opCtx, collection ) ) {}
             ...
             * foreground.
             */
            inline void allowBackgroundBuilding() { return this->_impl().allowBackgroundBuilding(); }
             ...
             */
            inline void allowInterruption() { return this->_impl().allowInterruption(); }
             ...
             */
            inline void ignoreUniqueConstraint() { return this->_impl().ignoreUniqueConstraint(); }

            /**
             * Removes pre-existing indexes from 'specs'. If this isn't done, init() may fail with
             * IndexAlreadyExists.
             */
            inline void removeExistingIndexes( std::vector< BSONObj > *const specs ) const { return this->_impl().removeExistingIndexes( specs ); }
             ...
             */
            inline StatusWith< std::vector< BSONObj > > init( const std::vector< BSONObj > &specs ) { return this->_impl().init( specs ); }

            inline StatusWith< std::vector< BSONObj > > init( const BSONObj &spec ) { return this->_impl().init( spec ); }
             ...
             */
            inline Status insertAllDocumentsInCollection( std::set< RecordId > *const dupsOut= nullptr ) { return this->_impl().insertAllDocumentsInCollection( dupsOut ); }
             ...
             */
            inline Status insert( const BSONObj &wholeDocument, const RecordId &loc ) { return this->_impl().insert( wholeDocument, loc ); }
             ...
             * Should not be called inside of a WriteUnitOfWork.
             */
            inline Status doneInserting( std::set< RecordId > *const dupsOut= nullptr ) { return this->_impl().doneInserting( dupsOut ); }
             ...
             * Requires holding an exclusive database lock.
             */
            inline void commit() { return this->_impl().commit(); }
             ...
             * back.
             */
            inline void abortWithoutCleanup() { return this->_impl().abortWithoutCleanup(); }

            inline bool getBuildInBackground() const { return this->_impl().getBuildInBackground(); }
    };
}  // namespace mongo
