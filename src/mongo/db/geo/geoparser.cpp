...
Status GeoParser::parseMultiLine(const BSONObj& obj, bool skipValidation, MultiLineWithCRS* out) {
    ...
    // Iterate array
    while (it.more()) {
        out->lines.push_back(std::make_unique<S2Polyline>());
        status = parseGeoJSONLineCoordinates(it.next(), skipValidation, out->lines.back().get());
        if (!status.isOK())
            return status;
    }
    if (0 == out->lines.size())
...
Status GeoParser::parseMultiPolygon(const BSONObj& obj,
    ...
    // Iterate array
    while (it.more()) {
        out->polygons.push_back(std::make_unique<S2Polygon>());
        status =
            parseGeoJSONPolygonCoordinates(it.next(), skipValidation, out->polygons.back().get());
        if (!status.isOK())
            return status;
    }
    if (0 == out->polygons.size())
