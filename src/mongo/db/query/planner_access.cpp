...
namespace
{
	 ...
	 */
	bool
	isTextNode( const std::unique_ptr< QuerySolutionNode > &node )
	{
	 ...
	 */
	FetchNode *
	getFetchNode( QuerySolutionNode *node )
	{
	 ...
	 */
	const IndexScanNode *
	getIndexScanNode( const QuerySolutionNode *node )
	{
	 ...
	 */
	bool
	scansAreEquivalent( const QuerySolutionNode *lhs, const QuerySolutionNode *rhs )
	{
...
namespace mongo
{
	...
	QueryPlannerAccess::makeCollectionScan(
	...
	bool
	QueryPlannerAccess::shouldMergeWithLeaf( const MatchExpression *expr,
	...
	void
	QueryPlannerAccess::mergeWithLeafNode( MatchExpression *expr, ScanBuildingState *scanState )
	{
	...
	// static
	void
	QueryPlannerAccess::finishTextNode( QuerySolutionNode *node, const IndexEntry &index )
	{
		...
		else
		{
			...
			std::vector< std::unique_ptr< MatchExpression > > prefixExprs( prefixEnd );
			...
			while( curChild < amExpr->numChildren() )
			{
				...
				prefixExprs[ ixtag->pos ]= amExpr->releaseChild( curChild );
				auto children= amExpr->releaseChildren();
				children.erase( children.begin() + curChild );
				amExpr->resetChildren( std::move( children ) );

				// Don't increment curChild.
			}

			// Go through the prefix equalities in order and create an index prefix out of them.
			for( auto &prefixMe : prefixExprs )
			{
				invariant( nullptr != prefixMe );
				invariant( MatchExpression::EQ == prefixMe->matchType() );
				EqualityMatchExpression *eqExpr= static_cast< EqualityMatchExpression * >( prefixMe.get() );
				prefixBob.append( eqExpr->getData() );
			}
			...
			else if( 1 == amExpr->numChildren() )
			{
				...
				std::unique_ptr< MatchExpression > child= std::move( amExpr->releaseChildren()[ 0 ] );
	...
	// static
	bool
	QueryPlannerAccess::orNeedsFetch( const ScanBuildingState *scanState )
	{
	...
	// static
	void
	QueryPlannerAccess::finishAndOutputLeaf( ScanBuildingState *scanState,
	...
	// static
	void
	QueryPlannerAccess::finishLeafNode( QuerySolutionNode *node, const IndexEntry &index )
	{
		const StageType type= node->getType();
	...
	// static
	void
	QueryPlannerAccess::findElemMatchChildren( const MatchExpression *node,
			vector< MatchExpression * > *out, vector< MatchExpression * > *subnodesOut )
	{
	...
	QueryPlannerAccess::collapseEquivalentScans(
			std::vector< std::unique_ptr< QuerySolutionNode > > scans )
	{
		std::vector< std::unique_ptr< QuerySolutionNode > > ownedScans= std::move( scans );
		...
		for( size_t i= 1; i < ownedScans.size(); ++i )
		{
			if( scansAreEquivalent( collapsedScans.back().get(), ownedScans[ i ].get() ) )
			{
				...
				// OrMatchExpression.
				auto collapsedFilter= stdx::make_unique< OrMatchExpression >();
				...
				collapseIntoFetch->filter= CanonicalQuery::normalizeTree( std::move( collapsedFilter ) );
			}
			else
			{
	...
	// static
	bool
	QueryPlannerAccess::processIndexScans( const CanonicalQuery &query, MatchExpression *root,
			bool inArrayOperator, const std::vector< IndexEntry > &indices,
			const QueryPlannerParams &params, std::vector< std::unique_ptr< QuerySolutionNode > > *out )
	{
		// Initialize the ScanBuildingState.
		ScanBuildingState scanState( root, inArrayOperator, indices );
		assert( ( scanState.curChild < root->numChildren() ) || ( NULL != scanState.currentScan.get() ) );
	...
	// static
	bool
	QueryPlannerAccess::processIndexScansElemMatch( const CanonicalQuery &query,
	        ScanBuildingState *scanState, const QueryPlannerParams &params,
	        std::vector< std::unique_ptr< QuerySolutionNode > > *out )
	{
		...
		for( size_t i= 0; i < emSubnodes.size(); ++i )
		{
			...
			if( !Indexability::isBoundsGenerating( subnode ) )
			{
				...
				auto childSolution= buildIndexedDataAccess( query, subnode, true, indices, params );
	...
	// static
	bool
	QueryPlannerAccess::processIndexScansSubnode(
		const CanonicalQuery &query,
		ScanBuildingState *scanState,
		const QueryPlannerParams &params,
		std::vector< std::unique_ptr< QuerySolutionNode > > *out )
	{
		//TODO -- All of this is borked!  The stealing of the pointer, and then the synthesis of the
		// ownership transfer.  All because buildIndexedDataAccess doesn't ALWAYS take ownership.
		// Change it to *ALWAYS* take ownerhship
		MatchExpression *root= scanState->root;
		MatchExpression *child= root->getChild( scanState->curChild );
		...
		else if( !inArrayOperator )
		{
			...
			// child.
			auto children= root->releaseChildren();
			ownedChild= std::move( children[ scanState->curChild ] );
			children.erase( children.begin() + scanState->curChild );
			root->resetChildren( std::move( children ) );
		...
		// child from root.
		auto childSolution= buildIndexedDataAccess( query, child, inArrayOperator, indices, params );
	...
	// static
	std::unique_ptr< QuerySolutionNode >
	QueryPlannerAccess::buildIndexedAnd(
		const CanonicalQuery &query,
		...
		const QueryPlannerParams &params )
	{
		...
		if( !processIndexScans( query, root.get(), inArrayOperator, indices, params, &ixscanNodes ) )
		{
			return nullptr;
		}
		...
		{
			auto fetch= stdx::make_unique< FetchNode >();
			verify( NULL != root.get() );

			if( root->numChildren() == 1 )
			{
				// An $and of one thing is that thing.
				auto children= root->releaseChildren();
				std::unique_ptr< MatchExpression > child= std::move( children[ 0 ] );

				// Takes ownership.
				fetch->filter= std::move( child );

				// 'root' deletes the empty $and.
			}
			else	// root->numChildren() > 1
			{// Takes ownership.
				fetch->filter= std::move( root );
			}
	...
	// static
	std::unique_ptr< QuerySolutionNode >
	QueryPlannerAccess::buildIndexedOr( const CanonicalQuery &query,
			std::unique_ptr< MatchExpression > root, bool inArrayOperator,
		...
		const QueryPlannerParams &params )
	{
		if( root->isLogical() && !Indexability::isBoundsGeneratingNot( root ) )
		{
			if( MatchExpression::AND == root->matchType() )
			{
				...
				return buildIndexedAnd(
					query, std::unique_ptr< MatchExpression >{ root }, inArrayOperator, indices, params );
			}
			else if( MatchExpression::OR == root->matchType() )
			{
				// Takes ownership of root.
				return buildIndexedOr(
					query, std::unique_ptr< MatchExpression >{ root }, inArrayOperator, indices, params );
			}
			else
			{
		...
		else
		{
			...
			else if( Indexability::isBoundsGenerating( root ) )
			{
				        ...
				        !indices[ tag->index ].multikey )
				{
					...
					soln->filter= std::move( autoRoot );
					return soln;
				}
				else
				{
					auto fetch= stdx::make_unique< FetchNode >();
					verify( nullptr != autoRoot.get() );
					fetch->filter= std::move( autoRoot );
	...
	std::unique_ptr< QuerySolutionNode >
	QueryPlannerAccess::scanWholeIndex(
		const IndexEntry &index,
		const CanonicalQuery &query,
		const QueryPlannerParams &params,
		int direction )
	{
		...
		auto isn= make_unique< IndexScanNode >( index );
		isn->maxScan= query.getQueryRequest().getMaxScan();
		isn->addKeyMetadata= query.getQueryRequest().returnKey();
		isn->queryCollator= query.getCollator();
		...
		else
		{
			...
			auto fetch= make_unique< FetchNode >();
	...
	// static
	void
	QueryPlannerAccess::addFilterToSolutionNode( QuerySolutionNode *node,
	...
	// static
	void
	QueryPlannerAccess::handleFilter( ScanBuildingState *scanState )
	{
		if( MatchExpression::OR == scanState->root->matchType() )
		{
	...
	// static
	void
	QueryPlannerAccess::handleFilterOr( ScanBuildingState *scanState )
	{
		...
		{
			MatchExpression *root= scanState->root;
			auto child= root->releaseChild( scanState->curChild );
			...
			{
				auto children= scanState->curOr->releaseChildren();
				children.push_back( std::move( child ) );
				scanState->curOr->resetChildren( std::move( children ) );
			}
		}
	}
	...
	// static
	void
	QueryPlannerAccess::handleFilterAnd( ScanBuildingState *scanState )
	{
		MatchExpression *root= scanState->root;
		const IndexEntry &index= scanState->indices[ scanState->currentIndexNumber ];
	...
	std::unique_ptr< QuerySolutionNode >
	QueryPlannerAccess::makeIndexScan(
		const IndexEntry &index,
		const CanonicalQuery &query,
		const QueryPlannerParams &params,
