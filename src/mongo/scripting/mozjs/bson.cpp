...
const JSFunctionSpec BSONInfo::freeFunctions[4] = {
    ...
    MONGO_ATTACH_JS_FUNCTION(usedDocumentSequences_forTesting),
    JS_FS_END,
};
...
void BSONInfo::Functions::usedDocumentSequences_forTesting::call(JSContext* cx, JS::CallArgs args) {
    if (args.length() != 1)
        uasserted(ErrorCodes::BadValue, "usedDocumentSequences_forTesting needs 1 argument");

    if (!getScope(cx)->getProto<BSONInfo>().instanceOf(args.get(0)))
        uasserted(
            ErrorCodes::BadValue,
            "argument to usedDocumentSequences_forTesting must be an object of type BSONInfo");
