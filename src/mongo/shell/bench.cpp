...
namespace
{
	inline pcrecpp::RE_Options
	flags2options( const char *flags )
	{
		...
		while( flags && *flags )
		{
			if( *flags == 'i' ) options.set_caseless( true );
			else if( *flags == 'm' ) options.set_multiline( true );
			else if( *flags == 'x' ) options.set_extended( true );
			flags++;
		}
		return options;
	}
...
namespace mongo
{
	...
	void
	BenchRunEventCounter::updateFrom( const BenchRunEventCounter &other )
	{
	...
	void
	BenchRunStats::updateFrom( const BenchRunStats &other )
	{
		if( other.error ) error= true;
	...
	void
	BenchRunConfig::initializeToDefaults()
	{
		host= "localhost";
		db= "test";
		username= "";
	...
	std::unique_ptr< BenchRunConfig >
	BenchRunConfig::createFromBson( const BSONObj &args )
	{
		auto config= stdx::make_unique< BenchRunConfig >();
		config->initializeFromBson( args );
		return config;
	}

	BenchRunOp
	opFromBson( const BSONObj &op )
	{
		BenchRunOp myOp;
	...
	void
	BenchRunConfig::initializeFromBson( const BSONObj &args )
	{
		initializeToDefaults();
	...
	DBClientBase *
	BenchRunConfig::createConnection() const
	{
	...
	void
	BenchRunState::waitForState( State awaitedState )
	{
	...
	void
	BenchRunState::tellWorkersToFinish()
	{
		_isShuttingDown.store( 1 );
	}

	void
	BenchRunState::tellWorkersToCollectStats()
	{
		_isCollectingStats.store( 1 );
	}

	void
	BenchRunState::assertFinished()
	{
	...
	bool
	BenchRunState::shouldWorkerFinish()
	{
		return ( _isShuttingDown.loadRelaxed() == 1 );
	}

	bool
	BenchRunState::shouldWorkerCollectStats()
	{
		return ( _isCollectingStats.loadRelaxed() == 1 );
	}

	void
	BenchRunState::onWorkerStarted()
	{
		stdx::lock_guard< stdx::mutex > lk( _mutex );
	...
	void
	BenchRunState::onWorkerFinished()
	{
		stdx::lock_guard< stdx::mutex > lk( _mutex );
	...
	_hasSpecial( const BSONObj &obj )
	{
		...
		while( i.more() )
		{
			...
			if( _hasSpecial( e.Obj() ) ) return true;
		}
		return false;
	}

	static BSONObj
	fixQuery( const BSONObj &obj, BsonTemplateEvaluator &btl )
	{
		if( !_hasSpecial( obj ) ) return obj;
	 ...
	 */
	int
	runQueryWithReadCommands( DBClientBase *conn,
	...
	void
	BenchRunWorker::generateLoadOnConnection( DBClientBase *conn )
	{
		verify( conn );
		long long count= 0;
		mongo::Timer timer;
		...
		while( !shouldStop() )
		{
			for( const auto& op : _config->ops )
			{
				...
				try
				{
					switch( op.op )
					{
						...
						case OpType::FINDONE:
						{
							...
							if( !_config->hideResults || op.showResult ) log() << "Result from benchRun thread [findOne] : " << result;
						} break;
						...
						case OpType::COMMAND:
						{
							...
							if( op.useCheck )
							{
								...
								if( !_config->hideResults || op.showResult ) log() << "Result from benchRun thread [command] : " << result;
							}
						} break;
						...
						case OpType::FIND:
						{
							...
							if( !_config->hideResults || op.showResult ) log() << "Result from benchRun thread [query] : " << count;
						} break;
						...
						case OpType::UPDATE:
						{
							BSONObj result;
							{
								...
								else
								{
									...
									if( op.safe ) result= conn->getLastErrorDetailed();
								}
							}
							...
							if( op.safe )
							{
								...
								if( !_config->hideResults || op.showResult ) log() << "Result from benchRun thread [safe update] : " << result;
						...
						case OpType::INSERT:
						{
							...
							if( op.safe )
							{
								...
								if( !_config->hideResults || op.showResult ) log() << "Result from benchRun thread [safe insert] : " << result;
						...
						case OpType::REMOVE:
						{
							...
							if( op.safe )
							{
								...
								if( !_config->hideResults || op.showResult ) log() << "Result from benchRun thread [safe remove] : " << result;
				...
				catch( DBException &ex )
				{
					if( !_config->hideErrors || op.showError )
					{
						...
						if( ( !_config->watchPattern && _config->noWatchPattern &&
						        ...
						        ( _config->watchPattern && _config->noWatchPattern && yesWatch && !noWatch ) ) log() << "Error in benchRun thread for op "
					...
					if( !_config->handleErrors && !op.handleError ) return;

					stats.errCount++;
				}
				catch( ... )
				{
					if( !_config->hideErrors || op.showError ) log() << "Error in benchRun thread caused by unknown error for op "
						                                             << opTypeName.find( op.op )->second;
				...
				if( op.delay > 0 ) sleepmillis( op.delay );
			}
		}
    ...
    BenchRunner *BenchRunner::create( std::unique_ptr< BenchRunConfig >config )
	{
		std::unique_ptr< BenchRunner > runner( new BenchRunner( std::move( config ) ) );

		stdx::lock_guard< stdx::mutex > lk( _staticMutex );
		auto oid= runner->_oid;
		_activeRuns[ oid ]= std::move( runner );
		return _activeRuns[ oid ].get();
	}

	BenchRunner::BenchRunner( std::unique_ptr< BenchRunConfig > config ) : _brState( config->parallel ), _config( std::move( config ) )
	{
		_oid.init();
	}

	BenchRunner::~BenchRunner()= default;
    void create(std::unique_ptr< BenchRunConfig >config);
	...
	BenchRunner *
	BenchRunner::createWithConfig( const BSONObj &configArgs )
	{
		auto config= BenchRunConfig::createFromBson( configArgs );
		return BenchRunner::create( std::move( config ) );
	}

	std::unique_ptr< BenchRunner > &
	BenchRunner::get( OID oid )
	{
		stdx::lock_guard< stdx::mutex > lk( _staticMutex );
	...
	void
	BenchRunner::populateStats( BenchRunStats *stats )
	{
	...
	static void
	appendAverageMicrosIfAvailable( BSONObjBuilder &buf,
	        const std::string &name,
	        const BenchRunEventCounter &counter )
	{
		if( counter.getNumEvents() > 0 ) buf.append( name,
			        static_cast< double >( counter.getTotalTimeMicros() ) / counter.getNumEvents() );
	}

	BSONObj
	BenchRunner::finish( std::unique_ptr< BenchRunner > runner )
	{
		runner->stop();
	...
	std::map< OID, std::unique_ptr<BenchRunner > > BenchRunner::_activeRuns;

	/**
	 * benchRun( { ops : [] , host : XXX , db : XXXX , parallel : 5 , seconds : 5 }
	 */
	BSONObj
	BenchRunner::benchRunSync( const BSONObj &argsFake, void *data )
	{
		BSONObj start= benchStart( argsFake, data );

		OID oid= OID( start.firstElement().String() );
		BenchRunner *runner= BenchRunner::get( oid ).get();
	 ...
	 */
	BSONObj
	BenchRunner::benchStart( const BSONObj &argsFake, void *data )
	{
	 ...
	 */
	BSONObj
	BenchRunner::benchFinish( const BSONObj &argsFake, void *data )
	{
		OID oid= OID( argsFake.firstElement().String() );

		// Get old BenchRunner object
		std::unique_ptr< BenchRunner > runner= std::move( BenchRunner::get( oid ) );

		BSONObj finalObj= BenchRunner::finish( std::move( runner ) );
