...
Future<SessionHandle> TransportLayerASIO::asyncConnect(HostAndPort peer,
    ...
    struct AsyncConnectState {
        AsyncConnectState(HostAndPort peer, asio::io_context& context,Promise<SessionHandle> promise_)
            : promise(std::move(promise_)), socket(context), timeoutTimer(context), resolver(context), peer(std::move(peer)) {}
    ...
    auto connector = std::make_shared<AsyncConnectState>(std::move(peer), *reactorImpl, std::move(pf.promise));
